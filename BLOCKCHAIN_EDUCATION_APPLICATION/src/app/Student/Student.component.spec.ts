...
describe('StudentComponent', () => {
  ...
  it('should update the table when a Student is added', fakeAsync(() => {
    ...
    sinon.stub(component.serviceStudent, 'addParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Student is updated', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceStudent, 'updateParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Student is deleted', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceStudent, 'deleteParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
